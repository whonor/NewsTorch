...
from models.modules.mmrec.utils_ import cached_path
...
class NewsEncoder(BertPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids.dim() == 3:
            ...
            num_regions = input_imgs.shape[2]
            feat_size = input_imgs.shape[3]
            input_imgs = input_imgs.view(-1, num_regions, feat_size)

            loc_size = image_loc.shape[3]
            image_loc = image_loc.view(-1, num_regions, loc_size)
            reshape_back = True
        else:
            reshape_back = False
        ...
        if reshape_back:
            pooled_output_t = pooled_output_t.view(batch_size, num_news, -1)
            pooled_output_v = pooled_output_v.view(batch_size, num_news, -1)
