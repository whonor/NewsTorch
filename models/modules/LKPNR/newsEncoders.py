...
import json
import random
from ..layers import Conv1D, Conv2D_Pool, MultiHeadAttention, Attention, QAttention, ScaledDotProduct_CandidateAttention, CandidateAttention
...
class NewsEncoder(nn.Module):
    def __init__(self, config: Config):
        ...
        with open('cache/word_embedding-' + str(config.word_threshold) + '-' + str(config.word_embedding_dim) + '-' + config.tokenizer + '-' + str(config.max_title_length) + '-' + str(config.max_abstract_length) + '-' + config.dataset_size + '.pkl', 'rb') as word_embedding_f:
