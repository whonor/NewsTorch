...
class ONCEDIRELlama1NewsEncoder(nn.Module):
    """
    LLaMA item encoder used by ONCE/DIRE.
    ...
    runs the remaining LLaMA layers, projects to the recommender hidden size,
    ...
    def __init__(self, config: Config):
        ...
        self.cache_key = getattr(config, "once_cache_key", "llama2")
        self.cache_root = getattr(config, "once_cache_dir", os.path.join(config.data_path, "once_dire_llama2_naml"))
        ...
        self.model_name_or_path = (
            os.environ.get("ONCE_LLAMA_MODEL", "")
            or os.environ.get("ONCE_LLAMA1_MODEL", "")
            or getattr(config, "once_lm_model_name_or_path", "")
        ...
        self.trust_remote_code = _as_bool(getattr(config, "once_trust_remote_code", True))
    ...
    def _raise_gated_checkpoint_error(self):
        raise RuntimeError(
            "Cannot access the gated LLaMA checkpoint. Request access on Hugging Face, then run "
            "`huggingface-cli login` or export HF_TOKEN/ONCE_HF_TOKEN with an approved token. "
            "Alternatively set ONCE_LLAMA_MODEL to a local checkpoint path."
        ) from None
    ...
    def set_corpus(self, corpus):
        ...
        if self.tune_from is None:
            raise ValueError(
                "ONCE-DIRE-LLAMA-NAML requires once_tune_from to cache frozen LLaMA layers. "
                "Use the paper setting once_tune_from: -2 in config/once.yaml."
            )
    ...
    def _load_transformer(self):
        ...
        try:
            ...
            raise ImportError("transformers is required for ONCE-DIRE-LLAMA-NAML.") from exc
        ...
        try:
            ...
            if any(term in str(exc).lower() for term in ["gated repo", "401", "unauthorized", "restricted"]):
                self._raise_gated_checkpoint_error()
            raise
    ...
    def _normalize_tune_from(self):
        ...
        if self.tune_from < 0 or self.tune_from >= num_layers:
            raise ValueError(f"once_tune_from must resolve into [0, {num_layers - 1}], got {self.tune_from}.")
    ...
    def _build_hidden_cache(self, corpus):
        try:
            ...
            raise ImportError("transformers is required to build the ONCE LLaMA hidden-state cache.") from exc
        ...
        if tokenizer.pad_token_id is None:
            raise ValueError("The LLaMA tokenizer needs a pad, eos, or unk token for ONCE cache generation.")
        ...
        with torch.no_grad():
            for start in range(0, len(prompts), self.page_size):
                ...
                batch_hidden = outputs.hidden_states[self.tune_from].float().cpu()
    ...
    def _collect_prompts(self, corpus):
        ...
        raise NotImplementedError(f"ONCE-DIRE-LLAMA-NAML does not support dataset_name={dataset_name!r}.")
    ...
    def _slice_transformer_layers(self):
        ...
        if layers is None:
            raise AttributeError("Expected a LLaMA-style AutoModel with a top-level .layers ModuleList.")
    ...
    def encode_by_index(self, news_index) -> torch.Tensor:
        if self.hidden_cache is None or self.mask_cache is None or self.transformer is None:
            raise RuntimeError("ONCE-DIRE LLaMA cache is not prepared. set_corpus(corpus) must run before training.")
...
class ONCE_DIRE_LLAMA1_NAML(nn.Module):
    def __init__(self, config: Config):
        super().__init__()
        self.news_encoder = ONCEDIRELlama1NewsEncoder(config)
    ...
    def forward(
        ...
        if history_index is None or news_index is None:
            raise ValueError("ONCE-DIRE-LLAMA1-NAML requires history_index and news_index from the dataset.")
