...
| SentiRec | SentiRec: Sentiment Diversity-aware Neural News Recommendation | 2020 | Diversity-aware | models/SentiRec.py models/modules/sentirec general_runner.py | config/sentirec.yaml | Uses sentiment prediction and diversity regularization with a dedicated trainer. |
| SentiDebias | Removing AI's Sentiment Manipulation of Personalized News Delivery | 2022 | Fairness-based | models/SentiDebias.py models/modules/senti_debias general_runner.py | config/sentidebias.yaml | Uses decomposed adversarial learning with a dedicated generator/discriminator trainer. |
| MMRec | MM-Rec: Visiolinguistic Model Empowered Multimodal News Recommendation | 2022 | Multimodal | models/MMRec.py models/modules/mmrec general_runner.py | config/mmrec.yaml | Requires cached news-image embeddings and uses a dedicated multimodal trainer. |
...
| CNRCL | A Contrastive News Recommendation Framework Based on Curriculum Learning | 2025 | Contrastive | models/CNRCL.py models/modules/cnrcl general_runner.py | config/cnrcl.yaml | Uses curriculum negative sampling, supervised contrastive learning, and a dedicated PCGrad trainer. |
| CPRS | User Modeling with Click Preference and Reading Satisfaction for News Recommendation | 2020 | Behavior-aware | models/CPRS.py dataset_corpus_preprocessing/EBNeRD_corpus_CPRS.py general_runner.py | config/cprs.yaml | Uses click preference and post-click reading-time satisfaction signals. |
| TCCM | TCCM: Time and Content-Aware Causal Model for Unbiased News Recommendation | 2023 | Causal | models/TCCM.py dataset_corpus_preprocessing/EBNeRD_corpus_TCCM.py general_runner.py | config/tccm.yaml | Requires time-aware word/entity popularity features from the dedicated EB-NeRD corpus. |
| SEIN | Modeling Stage-wise Evolution of User Interests for News Recommendation | 2026 | Temporal graph | models/SEIN.py dataset_corpus_preprocessing/EBNeRD_corpus_SEIN.py general_runner.py | config/sein.yaml | Combines global interaction graphs with stage-wise LSTM and self-attention interest modeling. |
