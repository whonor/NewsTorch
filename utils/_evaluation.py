...
from utils._beyond_accuracy import category_metrics_from_corpus
...
def _get_model_inputs(config, data_batch):
    ...
    if config.model == 'SentiDebias':
        ...
        if config.dataset_name == 'MIND':
            # 0-9: user features, 10-17: candidate features,
            # 18-19: sentiment scores, 20-21: news indices.
            return tuple(
                list(data_batch[:10])
                + [None, None, None]
                + list(data_batch[10:18])
                + [discretize(data_batch[18]), discretize(data_batch[19])]
            )
        if config.dataset_name in {'ebnerd', 'Adressa'}:
            # 0-20: standard features, 21-22: sentiment scores.
            return tuple(
                list(data_batch[:10])
                + [None, None, None]
                + list(data_batch[13:21])
                + [discretize(data_batch[21]), discretize(data_batch[22])]
            )
        raise ValueError(
            f"SentiDebias profiling is not supported for dataset {config.dataset_name!r}."
        )
...
def _build_sub_scores(indices, scores):
    if not indices:
        return []
...
def compute_scores_IPNR(config: Config, model: nn.Module, mind_corpus: MIND_Corpus, batch_size: int, mode: str, result_file: str, dataset: str):
    ...
    if dataset != 'submission' or mode != 'test':
        ...
        category_metrics = category_metrics_from_corpus(
            ...
            scores,
            top_ks=(5, 10),
            aspect="subcategory" if config.dataset_name == "Adressa" else "category",
        )
        return (auc, mrr, ndcg5, ndcg10, mae, rmse, recall5, recall10,
                hit5, hit10, precision5, precision10, *category_metrics)
    else:
        return (None,) * 16
